...
"""
RAPID, LASTFM, INSTAGRAM Y TIKTOK LO PODEMOS REEMPLAZAR
CON EL CHARTMETRIC IPI (LÍNEA 44 A 51)
"""
...
# Base de datos | HAY QUE INCORPORARLA A LA DB EXISTENTE
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///songxs.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# # NUEVO: Secret key para sesiones | PODEMOS QUITARLO Y USAR LO EXISTENTE
...
# EL LOGIN PODEMOS BORRARLO EN SU TOTALIDAD.
# EL USUARIO YA ESTARÁ LOGUEADO / LÍNEA 70 A 81)
# ============================================
...
# PODEMOS BORRAR LAS FEATURES EXTENDIDAS | LÍNEA 95 A 136
# ============================================
...
#         artist = sp.artist(artist_id)
        ...
        # GRAN FEATURE. ATRACTIVA. DEJEMOSLA A EXEPCIÓN DEL POPULARITY
        # POPULARITY (LINEA 148) NO NOS IMPORTA
        artist_image = None
...
# LAST FM Y DEMÁS APIS LO ELIMINAMOS (LINEA 156 A 450)
# PUEDE INCORPORARSE CON CHARTMETRIC Y NO LO NECESITAMOS PARA LA INGESTA
...
# ============================================
# RUTAS: Autenticación
# YA TENEMOS AUTH EN LA PLATAFORMA. PUEDE QUITARSE
# QUITAMOS HASTA LINEA 569
# ============================================
...
# PUEDE FUNCIONAR. HAY QUE REVISAR QUE ESTÉ IMPLEMENTADO
# ============================================
...
def extract_from_playlist():
    ...
    - EL COMPLETE MODE NO LO NECESITAMOS. PODEMOS CAMBIAR POR CHARTMETRIC.
    - EL COMPLETE MODE EXTRAE DATOS DE LOS APIS DE MUSIC BRAINZ, LAST.FM, INSTAGRA, ETC
    """
    try:
        data = request.get_json()
        ...
        # ESTE SI LO NECESITAMOS, ES LO MAS IMPORTANTE
        # ============================================
        playlist_id = None
        ...
        for index, item in enumerate(all_track_items):
            ...
            try:
                ...
                # ============================================
                # OPTIMIZADO: Usar caché de artista. Guardamos info del artista para no buscar dos veces.
                # ============================================
                if first_artist_id:
        ...
        for isrc in isrcs:
            try:
                ...
                if results['tracks']['items']:
                    ...
                    # NO ES IMPORTANTE. SUSTITUIR POR CHARTMETRIC
                    # try:
                    ...
                    #  NO NECESITAMOS. SUSTITUIR POR CHARTMETRIC
                    # try:
                    ...
                    # Obtener TikTok
                    # NO NECESITAMOS. USAR CHARTMETRIC O API TIKTOK
                    # try:
                    #     first_artist_name = track['artists'][0]['name']
...
def download_excel():
    ...
    # FUNCION QUE PUEDE SERVIR. IMPORTANTE
    try:
        data = request.get_json()
...
# IMPORTANTE, BUENA IMPLEMENTACIÓN
# ============================================
...
def export_excel():
    ...
    # LO HACE DESDE LA PESTAÑA DE MY CATALOG, NO DESDE EL PASO DE INGESTION
    try:
...
def export_pdf():
    ...
    # IMPORTANTE. REEMPLAZAR POR LOGOS DE ACRYLIC
        try:
        from io import BytesIO
        ...
        # Add logo if exists
        # ADD ACRYLIC.PNG LOGO. IMPORT FROM STATIC
...
# IMPORTANTE PERO YA ESTA IMPLEMENTADO EN EL CODIGO DE ACRYLIC.LA. 
# REVISAR QUE PODAMOS USAR EL TIER PRICE ORIGINAL
# ============================================
...
# ============================================
# RUTAS: Split Sheets
# YA TENEMOS SPLIT SHEETS EN EL ORIGINAL. REVISARLO Y SOLO UTILIZAR UNO
# ============================================
...
#####################################
# USUARIOS ADMINS VEN SPLIT SHEETS
# PODEMOS IMPLEMENTAR ESO. REVISAR QUE EL CODIGO ORIGINAL YA TENGA LA FUNCION
#####################################
...
##############
# Procesa la firma de un colaborador. SPLIT SHEET
##############
